class Solution {
    ...
    longestSubarray(nums, limit) {
        ...
        for (let r = 0; r < nums.length; r++) {
            while (max_deque.length > 0 && nums[r] > max_deque[max_deque.length - 1]) {
                max_deque.pop();
            }
            max_deque.push(nums[r]);
            console.log("max: ", max_deque, ptr_max_deque, nums.slice(l, r + 1));

            while (min_deque.length > 0 && nums[r] < min_deque[min_deque.length - 1]) {
                min_deque.pop();
            }
            ...
            while (max_deque[0] - min_deque[0] > limit) {
                ...
                if (nums[l] === max_deque[0]) max_deque.shift();
                // if (nums[l] === min_deque[ptr_min_deque]) ptr_min_deque += 1;
                if (nums[l] === min_deque[0]) min_deque.shift();
                l += 1;
            }
